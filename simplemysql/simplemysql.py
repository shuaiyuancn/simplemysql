...
class SimpleMysql:
    ...
    def __init__(self, **kwargs):
        ...
        self.conf['cursorclass'] = kwargs.get(
            'cursorclass', cursors.SSDictCursor  # use server side Dict cursor
        )
    ...
    def connect(self):
        ...
        try:
            self.conn = MySQLdb.connect(
                ...
                cursorclass=self.conf['cursorclass']
            )
    ...
    def getOne(self, table=None, fields='*', where=None, order=None,
               limit=(0, 1)):
        """Get a single result
    ...
    def leftJoin(self, tables=(), fields=(), join_fields=(), where=None,
                 order=None, limit=None):
        """Run an inner left join query

			tables = (table1, table2)
			fields = ([fields from table1], [fields from table 2])  # fields
			to select
			join_fields = (field1, field2)  # fields to join. field1 belongs
			to table1 and field2 belongs to table 2
			where = ('parameterizedstatement', [parameters])
					eg: ('id=%s and name=%s', [1, 'test'])
			order = [field, ASC|DESC]
			limit = [limit1, limit2]
    ...
    def update(self, table, data, where=None):
        ...
        return self.query(sql, data.values() + where[1] if where and len(
    ...
    def delete(self, table, where=None):
        ...
        return self.query(sql, where[1] if where and len(
            where) > 1 else None).rowcount
    ...
    def iterQuery(self, sql, params=None):
        """Iterate the result of a (SELECT) query with the help of the server
        side cursor
        """
    ...
    def _select_join(self, tables=(), fields=(), join_fields=(), where=None,
                     order=None, limit=None):
        """Run an inner left join query"""
